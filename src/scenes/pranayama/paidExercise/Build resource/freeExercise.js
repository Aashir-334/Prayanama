...
import CountDown from '../../../../components/countDown/paidExercise/buildResource';
...
class freeExercise extends Component {
  ...
  toggleModal = () => {
    ...
    if (
      ...
      Tts.speak(store.getState().language.buildResourceExercise, {
        androidParams: {
          KEY_PARAM_PAN: 0,
          KEY_PARAM_VOLUME: 1,
  ...
  onPauseSound = () => {
    if (!this.state.pause) {
      ...
      // this.passPause();
      // this.animationOn()
      song.stop(() => ToastAndroid.show(store.getState().language.thanks, ToastAndroid.BOTTOM));
                  ...
                  >
                    <View style={style.infoText}>
                      <Text style={style.insideHeading}>
                        {store.getState().language.buildResourceExercise}
                      </Text>
                    </View>
                  </Animated.View>
                )
